...
class STBTableViewIndex: UIControl {
	...
	var controlFrame: CGRect {
		return CGRectMake(controlOriginX, view.frame.origin.y, controlSizeWidth, view.frame.size.height)
	}
	
	var controlBounds: CGRect {
		return CGRectMake(0.0, 0.0, controlSizeWidth, view.frame.size.height)
	}
...
//		view.frame = viewFrame
        view.frame = CGRectMake(5.0,0,16.0,view.frame.size.height)
...
//	}
	...
	func handleGesture(gesture: UIGestureRecognizer) {
        ...
        if visible == false {
            NSNotificationCenter.defaultCenter().postNotificationName("dissappearIndexViewNotification", object: self)
